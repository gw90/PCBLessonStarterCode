...
class ThtLed(Led, FootprintBlock):
    def contents(self):
        super().contents()
        self.footprint('D', 'LED_THT:LED_D3.0mm',
                       {
                           '1': self.k,
                           '2': self.a
                       },
...
# Power Connections Footprint
class Power_Tht2Pin(PowerBarrelJack, FootprintBlock):
    def contents(self):
        super().contents()
...
class Astable555Timer(GeneratorBlock):
    ...
    def generate(self) -> None:
        ...
        self.connect(self.pwr, self.rA.a.adapt_to(VoltageSink()), self.capBypass.pos.adapt_to(VoltageSink()))
        self.connect(self.gnd, self.cap.neg.adapt_to(Ground()), self.capBypass.neg.adapt_to(Ground()), self.capContBypass.neg.adapt_to(Ground()))
        self.connect(self.ne.cont, self.capContBypass.pos)
        self.connect(self.rA.b, self.rB.a, self.ne.disch)
        self.connect(self.rB.b, self.ne.trig, self.ne.thres, self.cap.pos)
